...
This tutorial focuses more on the 'how', offering you some code chunks that you can modify (and reuse in future should you wish) to help you familiarize yourself with carrying out basic operations in R, the 'why' pertaining to many of the concepts touched on here will be covered in more detail later tutorials.
...
## Checking your data {#checking_your_data}
...
+ The `dim()` function gives the number of rows by columns in your table of data.

Using the dataset called `penguins` run the above functions look at the first 6 rows, to get the column names, and display the number of rows and columns:
...
```{r str_example, echo=TRUE}
str(penguins)
```

Note the different types of data listed:

+ Factors - `species`, `island`, `sex`. These are categorical variables that contain two or more classes
+ num or "Numeric" - `bill_length_mm`, `bill_depth_mm`. These are numerical variables with decimal places
+ Int or "Integer" - `year`, `flipper_length_mm`, `body_mass_g`. These are whole number numerical variables without decimals.
...
How can we calculate just the mean body weight, `body_mass_g`? Here, our "goal" is to calculate the overall mean, which is easy to do using `mean()` function from the `mosaic` package. As you only want to calculate an overall mean, the structure of your function is:

![](images/template_one_variable.png)

so you don't need to put a name to the left of the **~** (tilde) symbol in the template. Have a go at coding this up, with the correct names for "goal", "variable_name" and "mydata":

```{r mean-func, exercise = TRUE}

```
```{r mean-func-solution}
mean(~body_mass_g, data = penguins)
```

The output shows NA (short for "not available" or "missing" data). Looking at the outputs of `summary(penguins)` you can see that there are a number of missing records. We can add an argument to the `mean()` function to tell it to remove the NA's and calculate statistics based on the data available. This extra argument is `na.rm = TRUE`. Try and modify your code to test this in the box below:
...
# mean weight with NA (missing) records ignored
...
Here our goal is to calculate a simple summary statistic such as a mean or median. We can do this with the formula: 
...
Some of the most useful functions from the `mosaic` package are listed [here](https://rdrr.io/cran/mosaic/man/aggregating.html) , a particularly useful function you might want to checkout is `favstats()` that computes several statistics at once. We will use `favstats()` in Practical 1 so you will see how it works.
...
Lets take a look at the below dataset, which is called `datasaurus_dozen` which has x and y values for different groups (recorded as the dataset variable) which are recorded as a character variable. Recall that character variables are text-based rather than numeric-based, and you can use the `str()` function to look at your data structure. See [Checking your data](#checking_your_data)
...
We will now calculate some summary statistics for `x` and `y` using `dataset` as a grouping variable. Notice how all the calculations for mean or standard deviation follow the same structure:

<div id="datasaurus-structure">
$$\text{statistic(variable ~ group, data= dataset)}$$ 
</div>
...
# mean of x values in each group
mean(x~dataset, data = datasaurus_dozen) 
# mean of y values in each group
mean(y~dataset, data = datasaurus_dozen)

# sd of x values in each group
sd(x~dataset, data = datasaurus_dozen)
# sd of y values in each group
...
Let us plot all the `x` and `y` values in one scatterplot using the `gf_point()` function. We will use the option `colour = ~dataset` to colour the points by `dataset`.
...
If you are unsure of what plot would be best for your data, an excellent online resource is  [data-to-vis](https://www.data-to-viz.com/) which provides a flowchart to help make the best decision, and examples in R. 


## Visualizing single variables

Remember that a variable can be continuous numeric data (1.34, 5.87, 0.23 etc.) or categorical (SiteA, SiteA, SiteA, SiteB, SiteB, SiteC etc.). The way in which you may want to visualise these data is slightly different. Categorical variables are sometimes summarised in bar charts, but here we will focus on numeric data.

### Numeric data

When investigating a numerical variable we might want to look at the distribution of values (we will talk more about why this is important in Practical 1) using plots which we could do in a variety of different ways. We can use a the `gf_historgram()` function to produce a histogram which takes our numeric variable and cuts our data into "bins" and counts the frequency each value occurs in those bins.

```{r simple_histogram, exercise=TRUE}
gf_histogram(~body_mass_g, data = penguins)
```

Note that the plot also came with a warning message (remember we had a couple of data points with an NA value) just to let us know that there are some data that were missing and so aren't included in the plot because our plotting function didn't know where to put them.
...
We can represent the distribution of values in another way by doing a density plot using `gf_density()` function. A density plot is similar to a histogram but the data are a smoothed version. Notice the y-axis has now changed and is showing a probability density instead but the interpretation of how those data are distributed is still the same. Try modifying the following code to use the `gf_density()` function to see the difference:
...
## Bar charts of counts or proportions

### Bar chart with one categorical variable

For categorical data we might want to look at how many observations of each category we have using a bar chart. A bar chart is a good way to look at the numeric values for categorical data. Each group is represented by a bar and there are gaps between bars which is a key distinction between a bar chart and histogram. Here we are just looking at a single categorical variable, `island`, and counting how many records we have for each island:

```{r simple_bar, exercise=TRUE}
gf_bar(~island, data= penguins)
```

### Bar charts with multiple categorical variables 
...
By plotting proportional data, we can now see that nearly 50% of these penguin data have come from Biscoe Island. This is because the Biscoe Island bar is nearly at 0.5 which is 50%.

## Bar charts and boxplots
### More complex bar charts
It is very common to add symbols, usually lines above and below each bar, to indicate the amount of variation in the data. Typically these represent the _standard error_ around the mean. There are better ways of displaying this type of data such as boxplots (see below) but as bar charts are so common and you may be asked to present one, this is how to do it in R. Suppose you want to display the mean weight of the different species of penguin +/- their standard error (SE). You need to create a table of data like this:

```{r barchart_basics, exercise = TRUE}
spp_favstats  <- favstats(body_mass_g ~ species, data = penguins)
print(spp_favstats)
```

This gives us a column called `mean` containing the mean weight of each species (in g), and a column called `sd` with the standard deviation. Recall that you can calculate the SE by dividing the sd by the square root of the number of records, held in column `n`, simply as:

```{r barchart_se_calc-setup}
spp_favstats  <- favstats(body_mass_g ~ species, data = penguins)
```

```{r barchart_se_calc, exercise = TRUE}
spp_favstats$se <- spp_favstats$sd / sqrt(spp_favstats$n)
print(spp_favstats)
```

Now build up your barchart in steps. First create the barchart, using `gf_col()` which can accept any numeric variable on the left side of the `~` and expects a categorical variable on the right side. Here we have the mean weight on the left (continuous response) against the species (categorical explanatory):

```{r barchart_simple-setup}
spp_favstats  <- favstats(body_mass_g ~ species, data = penguins)
spp_favstats$se <- spp_favstats$sd / sqrt(spp_favstats$n)
```

```{r barchart_simple, exercise = TRUE}
gf_col(mean ~ species, data = spp_favstats)
```

Now, to add the standard error bar, we have to specify that we want a bar above (+) and below (-) the mean, using the `gf_error()` function, which we add to the plot:
...
```{r barchart_se, exercise = TRUE}
gf_col(mean ~ species, data = spp_favstats) %>%
  gf_errorbar((mean + se) + (mean - se) ~ species)
```

by default the error bars are a bit to wide, so use the `width` option to reduce their size, and smarten the plot with `gf_theme()`:

```{r barchart_smarter-setup}
spp_favstats  <- favstats(body_mass_g ~ species, data = penguins)
spp_favstats$se <- spp_favstats$sd / sqrt(spp_favstats$n)
```

```{r barchart_smarter, exercise = TRUE}
gf_col(mean ~ species, data = spp_favstats) %>%
  gf_errorbar((mean + se) + (mean - se) ~ species, width = 0.2) %>% 
  gf_theme(theme_classic())
```

### Boxplots
One weakness of barplots is that they do not give a full understanding of the variation around the mean, especially as by default they start at zero, as in the above plot, which can make the error bars hard to see (depends on noise in your data sample size, n) and they don't show outliers, or whether the distribution is skewed. Boxplots are therefore the recommended alternative, and are easy to create from the raw data:

```{r boxplot_example, exercise = TRUE}
gf_boxplot(body_mass_g ~ species, data = penguins) %>% 
  gf_theme(theme_classic())
```

This gives you a better understanding of the data: Adelie and Chinstrap penguins have lower body mass (although there are a couple of outliers in the latter). The distribution of data looks reasonably symmetrical for all three species. The horizontal line is your **median** value, 50% of your values fall within the box, and 75% within the 'whiskers', The dots are outliers. This diagram helps you understand what the lines and boxes represent in a boxplot <https://r-graph-gallery.com/boxplot>
...
```{r penguin-bill-colour, exercise = TRUE}
# colour by species
gf_point(bill_depth_mm ~ bill_length_mm, 
         colour = ~species,  
         data= penguins)
...
You have just created a plot which include colour and we can now see that there are differences in the relationship between bill_depth_mm and bill_length_mm for the different species. But we can make this figure tidier, more professional looking and get it "publication" ready.

```{r penguin-bill-publication, exercise = TRUE}
# colour by species
gf_point(bill_depth_mm ~ bill_length_mm, 
         colour = ~species,  
         data= penguins) %>% 
...
(click the "Solution" button to get the answer, but try it yourself first!)
