...
* `bivalve.csv` This contains the abundance of bivalves at two marine (coastal) sites on four transects
...
# Before you start
## Essential first steps before you do this practical
If you did the first practical last week these should be straightforward:

1. Click on the Windows command button (bottom left of screen), type in RStudio and start the software. (If asked to upgrade etc. ignore)
2. Inside RStudio on the main menu click on **File -> Recent Projects -> NES2504**
3. Look at the top-right of the RStudio window. You should see 'NES2504' displayed. **If it says 'Project: (None)' then please speak to a demonstrator.**

## Setting up a new R script to record your work
Now, on the RStudio main menu click on **File -> New File -> R Script** This should open a blank R script at the top left of your screen. Before you go any further, I suggest you type into your script a couple of lines of comments to explain what the script is being used for. As you will be using the `mosaic` R package you should also "activate" it to make its extra commands available, using the `library()` function:

```{r, eval = FALSE}
# Practical 1 : introduction to basic summary statistics and graphs using chick
# and bivalve data.

# Activate the mosaic package
library(mosaic)
```
...
One of the reasons why R is so popular is that it allows you to do **reproducible** research. Several weeks or even months may pass between your data analyses (steps 6 and 7), and writing your report (step 10) but your R script will provide a complete record of what you have done. Indeed, even the final report can be written in R, to automatically include relevant graphs, data analyses etc. All the Microsoft Word documents on the NES2504 Canvas pages are actually written in RStudio, and auto-converted to Word, so I can skip Step 9 above!

# Download and import data
## Download data files
Go to Canvas and download `chick_diet.csv`, `chick_growth.csv` and `bivalves.csv` . Remember that when you download anything from a browser like Chrome or Edge, in general it will be stored in your "Downloads" folder. See the practical material from last week if you have forgotten the details. Therefore, before you can import the data, you need to copy it to your NES2504 project folder:
...
* Navigate to your **NES2504/Data** folder, right-click in a blank area inside the **NES2504/Data** folder and select "Paste"

You should now see the following inside your **NES2504/Data** folder:

![](images/nes2504_data_folder.png)
...
You may also have a couple of other files displayed if you have already completed the Session 0 "In your own time" exercise on managing difficult Excel formats. Inside the **NES2504** folder you should now see something similar to:

![](images/nes2504_folder.png)
which probably contains:

* `Data` the subfolder shown above with your CSV files
* `NES2504` (full name `NES2504.RProj`). Your RStudio Project settings created in the previous practical
...
If you see other files called **.Rhistory** and **.RData** you can delete them as they are not needed.

**Where should you store your data files?** Some users put their R scripts and their data files in the same folder. However, it can quickly get confusing having a mixture of R scripts, CSV files and Excel files all jumbled together, so we recommend you store your CSV and Excel files in the `Data/` subfolder.

## Import your data into R
Go to your R script, called `practical1.R`, and edit it so that it reads as follows. Your script **must** contain a title, comments, blank lines, spaces around the `<-` assign symbol etc so that it is readable!!
...
# Read in bivalve abundance data for 4 marine sites, 2 transects
bivalves_dat <- read.csv("Data/bivalves.csv")
```

Save your script by selecting **File -> Save** from the main RStudio menu. Next, position your mouse cursor on the first of the three `read.csv` lines, and hit the <kbd>Ctrl</kbd> + <kbd>Enter</kbd> key combinations several times to send the commands to the RStudio Console where they will be run. If you receive an error message, check you've copied the CSV files over from Downloads to your NES2504 project folder, look for any spelling errors (R is case-sensitive too), check you've typed `read.csv` and not `read-csv`, make sure you've surrounded the CSV filenames with opening and closing `"` symbols, and finally ask a demonstrator.
...
**Reminder** Make sure to comment your script, using the `#` write some comments (notes) to annotate the script that you have just written so that you know what all the lines do. NOTE THERE ARE ALSO BLANK LINES BETWEEN COMMANDS TO MAKE THE SCRIPT MORE READABLE!
...
+ **y** = the name of the column containing the **response** variable
+ **x** = the name of the column containing an **explanatory** variable
+ **mydata** = name of your table of data, known in R as a **data.frame**
+ **...** = options
...
<center>
<h2><strong><span class="boxed">statistic</span> ( <span class="boxed">chick weights</span> ~ <span class="boxed">diet</span> , data = <span class="boxed">diet_dat</span>)</strong> 
</h2></center>
...
+ **statistic** = the statistic that we want to calculate, e.g. mean, median, sd, etc.
...
We can now replace the "statistic" above with what we want to calculate, and use the correct names `weight` and `diet` for the two columns in `diet_dat`. Add the following lines to the end of your `practical1.R` script and execute them:
...
Please look at [this explanation of variance, or var](https://naturalandenvironmentalscience.shinyapps.io/NES2504_SS/#section-variance-correcting-ss-for-the-number-of-records) and [this explanation of standard deviation, or sd](https://naturalandenvironmentalscience.shinyapps.io/NES2504_SS/#section-standard-deviation-correct-units) that introduce these measures of spread. If you do not understand variance and standard deviation, please speak to myself or one of the demonstrators. You can also use Microsoft Teams to send a message to all the demonstrators for advice.  Calculation is straightforward in R, using the functions `var()` and `sd()` and substituting them in as our "goals":
...
+ **mydata** = An R table of data `data.frame` that contains the x and y in columns
...
<center>
<h2><strong><span class="boxed">plot type</span> (~ <span class="boxed">horizontal axis</span> , data = <span class="boxed">mydata</span>, ... )</strong> 
</h2></center>
...
numbers of cases) on vertical y-axis for a continuous numeric variable on horizontal x-axis. The frequency is calculated for you automatically. Again, from a "goal-oriented" perspective it is:

&nbsp;

<center>
<h2><strong><span class="boxed">histogram</span> (  ~ <span class="boxed">weight</span> , data = <span class="boxed">diet_dat</span>)</strong> 
</h2></center>

&nbsp;


Let's look at the overall frequency histogram of your chick weights. Add the following lines to the end of your `practical1.R` script, save it. **Note** Remember to leave a few blank lines in your `practical1.R` script before beginning the commands for this plot, to make your script more readable. Run the new lines by hitting the <kbd>Ctrl</kbd> + <kbd>Enter</kbd> key combinations as usual:
...
<center>
<h2><strong><span class="boxed">boxplot</span> ( <span class="boxed">chick weights</span> ~ <span class="boxed">chick diet</span> , data = <span class="boxed">diet_dat</span>)</strong> 
</h2></center>

&nbsp;

which can be translated into R as:
...
At the moment the axis labels are just `weight` and `Diet`. Edit your code into something more meaningful by "chaining" on an additional line that begins `gf_labs()`, so the vertical y axis label reads "Weight (g)" as you must always include units where possible and the horizontal x-axis is "Experimental Diet". Once this works edit your plotting code to add a final, third line, that removes the grey background via `gf_theme()`. Please ask a demonstrator if you have problems.
...
You can see that the chicks get heavier with time, as expected. The problem with this graph is that it does not indicate that we actually have four different diets. So let's colour the points accordingly. Remember that diet is another *explanatory* variable; we have two explanatories we want to visualise, `time` and `diet`. So rather than putting `colour = diet` we use `colour = ~diet` to indicate that it is explanatory with the `~` or "tilde" symbol:
...
Note that writing `gf_point(weight~time + diet, data=growth_dat)` will give an error as `gf_point()` would not know whether to put `time` or `diet` on the x-axis. 
...
Later in the NES2504 course you will learn how to create fitted lines from models to add to your graphs.
...
Working in RStudio can seem challenging at first, but adhering to some basic guidelines helps build cleaner workflows:

* Store your R scripts separate from your CSV or Excel files, putting the latter in a `Data` subfolder
* Use the goal-oriented approach to help you code both simple statistics and plots
* When creating plots, begin with a simple plot, of e.g. response and explanatory variables only. If creating frequency histograms or density plots, you only need to include one variable.
* Once you have created a simple plot, use the "pipe" symbol **%>%** to indicate "THEN" to add extra lines below,
indenting the code, to improve axis labels, colours, error bars, overall "theme" etc.

# Bivalves data (in your own time)
## The problem: create coloured bar chart with error bars
This exercise builds on some of the material you have already learnt, and will give you greater confidence in your use of key skills in R for summary statistics and data visualisation. Import the CSV file into a `bivalves_dat` table in the usual way with `read.csv()`:

```{r, echo=TRUE, eval=FALSE}
bivalves_dat <- read.csv("bivalves.csv")
```

If you wish, try and calculate the additional summary statistics and create the graphs for the abundance of bivalves along two transects at four sites. We're now going to learn how to create a barchart based on the `bivalves_dat` dataset you imported earlier. This shows the mean numbers of bivalves (+/- standard error bars) at each site, colour-coded for each treatment:
...
* Remember to set the `colour` option correctly using the `~` symbol to indicate that `transect` is a response variable
...
## Create a table for barchart dataset
...
Check the contents of your `bivalves_stats` table to make sure it matches what is displayed above (either double-click on it in your RStudio Environment window at the top-right, or print it to the Console).

## Create a simple stacked barchart
We will use a stepwise approach to building your bar chart. First, a simple barplot:
...
The problem here is that this is a 'stacked' barchart: these are also popular in science, but is not what we want here. We want the coloured bars that represent each transect side-by-side. 

## "Dodge" the bars side-by-side
Putting the coloured bars for each transect side by side is called "dodging" the bars. So we'll use the `position_dodge` option to move the bars side-by-side:
